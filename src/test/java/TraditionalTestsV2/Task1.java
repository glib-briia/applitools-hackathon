...
/*
* Copy of TraditionalTestsV1.Task1 plus additional steps to catch missed out defects (steps 15-17)
* */
public class Task1 extends BaseTraditionalTest {
    ...
    @Test(testName = "Header navigation menu should be displayed", groups = {Laptop, Tablet, iPhone_X}, priority = 2)
    public void testNavigationMenuShouldBeDisplayed(ITestContext context) {
        assertThat(hackathonReporter(TASK_ID, "Header navigation menu should be displayed", context, galleryPage.header.navigationMenu,
    ...
    @Test(testName = "Filters facets should be hidden", groups = {Tablet, iPhone_X}, priority = 12)
    public void testFiltersShouldBeHidden(ITestContext context) {
        assertThat(hackathonReporter(TASK_ID, "Filters facets are hidden", context, galleryPage.filter_col, !galleryPage.filter_col.isDisplayed())).isTrue();
    }
    ...
    @Test(testName = "Desktop search icon should be hidden on Mobile", groups = {iPhone_X}, priority = 15)
    public void testDesktopSearchIconShouldBeHiddenOnMobile(ITestContext context) {
        assertThat(hackathonReporter(TASK_ID, "Desktop search icon is hidden on mobile", context, galleryPage.header.searchIcon,
                !galleryPage.header.searchIcon.isDisplayed())).isTrue();
    }

    @Test(testName = "'Filters' icon should be displayed", groups = {Tablet, iPhone_X}, priority = 16)
    public void testFiltersIconShouldBeDisplayed(ITestContext context) {
        assertThat(hackathonReporter(TASK_ID, "'Filters' icon is displayed", context, galleryPage.filtersIcon,
                galleryPage.filtersIcon.isDisplayed())).isTrue();
    }

    @Test(testName = "'Filters' icon should be hidden", groups = {Laptop}, priority = 16)
    public void testFiltersIconShouldBeHidden(ITestContext context) {
        assertThat(hackathonReporter(TASK_ID, "'Filters' icon is hidden", context, galleryPage.filtersIcon,
                !galleryPage.filtersIcon.isDisplayed())).isTrue();
    }

    @Test(testName = "Add to wishlist/cart/compare buttons should be displayed on all products", groups = {Tablet, iPhone_X}, priority = 17)
    public void testProductsShouldHaveCardWishListCompareIcons(ITestContext context) {
        SoftAssertions softly = new SoftAssertions();
        List<GalleryPage.Product> products = galleryPage.getProducts();
        for (int index = 0; index < products.size(); index++) {
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Wish list button is displayed for %d product", index), context, products.get(index).getAddToWishListBtn(),
                    products.get(index).getAddToWishListBtn().isDisplayed())).isTrue();
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Add to compare button is displayed for %d product", index), context,
                    products.get(index).getAddToCompareBtn(),
                    products.get(index).getAddToCompareBtn().isDisplayed())).isTrue();
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Add to cart button is displayed for %d product", index), context, products.get(index).getAddToCartBtn(),
                    products.get(index).getAddToCartBtn().isDisplayed())).isTrue();
        }
        softly.assertAll();
    }

    @Test(testName = "Add to wishlist/cart/compare buttons should be displayed on all products", groups = {Laptop}, priority = 17)
    public void testAddWishListCompareIconsShouldBeHidden(ITestContext context) {
        SoftAssertions softly = new SoftAssertions();
        List<GalleryPage.Product> products = galleryPage.getProducts();
        for (int index = 0; index < products.size(); index++) {
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Wish list button is hidden for %d product", index), context, products.get(index).getAddToWishListBtn(),
                    !products.get(index).getAddToWishListBtn().isDisplayed())).isTrue();
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Add to compare button is hidden for %d product", index), context, products.get(index).getAddToCompareBtn(),
                    !products.get(index).getAddToCompareBtn().isDisplayed())).isTrue();
            softly.assertThat(hackathonReporter(TASK_ID, String.format("Add to cart button is hidden for %d product", index), context, products.get(index).getAddToCartBtn(),
                    !products.get(index).getAddToCartBtn().isDisplayed())).isTrue();
        }
        softly.assertAll();
    }
